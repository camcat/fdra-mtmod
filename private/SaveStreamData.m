...
function A = Copy(fsrc,dst,cidxs,chunk_size)
  if(nargin < 4)
    nr = NbrRows(fsrc);
    % Limit amount read at a time.
    chunk_size = ceil(2^24/nr);
  end
  ssd = Init(dst);
  i = 1;
  while(i <= numel(cidxs))
    cis = i + (0 : chunk_size - 1);
    cis(cis > numel(cidxs)) = [];
    d = Read(fsrc,cidxs(cis));
    ssd = Write(ssd,d);
    i = cis(end) + 1;
  end
  Finalize(ssd);
...
function Test
  ...
  A0 = (1:10)'*(1:20);
  ssd = SaveStreamData('Write',ssd,A0);
